...
author: "Matthew Peverill, Justin D. Russell, Taylor J. Keding, Hailey M. Rich, Max A. Halvorson, Kevin M. King, Rasmus M. Birn, & Ryan J. Herringa"
date: "`r Sys.Date()`"
output:
  pdf_document:
    toc: true
...
Raw Body Mass Index (BMI) was calculated using measured height and weights using the standard formula:

$\frac{703*W}{H^2}$

Where W is the participants weight in lbs (averaged over up to 3 measurements) and H is the participant's height in inches. The raw value was then standardized, with adjustment for participant age and sex, using WHO norms as implemented by the R package zscorer (Myatt & Guevarra, 2019).
...
```{r}
supplement.tables.xlsx<-openxlsx::createWorkbook()
```
...
supplement.tables.xlsx<-as_Workbook(t1,
                     Workbook=supplement.tables.xlsx,
                     sheet="table1")
t1
```
...
supplement.tables.xlsx<-as_Workbook(t2,
                     Workbook=supplement.tables.xlsx,
                     sheet="table2")
...
## Relation of participant characteristics to scan time (post scrubbing)

Participants have a variable number of low-motion frames available for analysis, contingent on the chosen motion threshold and participant motion. This variability may lead to heterogeneity in the stability of functional connectivity estimates between participants. Although five minutes of 'good' scan time is conventionally considered sufficient for a stable estimate, the following analysis examines the relation of scan length (post-scrubbing) to participant characteristics. The results are present in supplemental tables i and ii, which are available in a spreadsheet in the source OSF repository (https://osf.io/57xer/).

```{r}
# The code is extremely similar to thaht used in the FD models, above. Only y_names is changed.
# Updated colnames for vars
y_names = c("s_remaining_0.1","s_remaining_0.2","s_remaining_0.3","s_remaining_0.4","s_remaining_0.5")
x_names = c("adi","coi","nihtbx_flanker_agecorrected","nihtbx_cryst_agecorrected","nihtbx_totalcomp_agecorrected",
             "pea_wiscv_tss","pfactor","INT","EXT","interview_age","bmiAgeZ","sex","household.income","p.edu",
             "race_ethnicity.factor","ksads_factor","pds_category")
...
# table of sd's for calculating standardized beta
sdseconds <- t(df %>%
  summarize(
    sd_seconds_0.1 = sd(s_remaining_0.1, na.rm = TRUE),
    sd_seconds_0.2 = sd(s_remaining_0.2, na.rm = TRUE),
    sd_seconds_0.3 = sd(s_remaining_0.3, na.rm = TRUE),
    sd_seconds_0.4 = sd(s_remaining_0.4, na.rm = TRUE),
    sd_seconds_0.5 = sd(s_remaining_0.5, na.rm = TRUE)
  ))
...
contHux <-contOut %>% 
  group_split(IV) %>% 
  bind_cols() %>%
  select(2,31:35,24:28,17:21,10:14,3:7) %>% 
  as_hux() %>% 
  set_contents(1,everywhere,c("DV",rep(c("b","β","r^2","t","p"),5))) %>% 
  insert_row(c("",rep(c(".5",".4",".3",".2",".1"),each=5))) %>%
  merge_across(1,2:6) %>% 
  merge_across(1,7:11) %>% 
  merge_across(1,12:16) %>% 
  merge_across(1,17:21) %>% 
  merge_across(1,22:26) %>%
  set_caption("Linear regression parameters for relation of discrete variables to post-scrubbing scan time available (in seconds)")
  

supplement.tables.xlsx<-as_Workbook(contHux,
                     Workbook=supplement.tables.xlsx,
                     sheet="table_i")

#Write discrete table
discOut$sd.seconds<-rep(sdseconds,each=sum(discOut$IV=="s_remaining_0.1"))
discOut$sBeta<-discOut$BETA/discOut$sd.seconds

subelements<-split(2:51,ceiling(seq_along(2:51)/10))

discHux<-discOut[,c(1:11,13)] %>% 
  group_split(IV) %>% 
  bind_cols() %>% 
  select(2,51:60,39:48,27:36,15:24,3:12) %>% 
  as_hux() %>% 
  set_contents(1,everywhere,c("DV",rep(c("F","df (1)","df (2)","Model p","Contrast","b","sd (seconds)","t","p","β"),5))) %>% 
  merge_repeated_rows(everywhere,c(1,unlist(lapply(subelements,\(x) {return(x[1:4])})))) %>% 
  insert_row(c("",rep(c(".5",".4",".3",".2",".1"),each=10))) %>% 
  merge_across(1,2:11) %>% 
  merge_across(1,12:21) %>% 
  merge_across(1,22:31) %>% 
  merge_across(1,32:41) %>% 
  merge_across(1,42:51) %>% 
  set_caption("F tests for relation of discrete variables to post-scrubbing scan time available (in seconds)")

supplement.tables.xlsx<-as_Workbook(discHux,
                     Workbook=supplement.tables.xlsx,
                     sheet="table_ii")
...
```{r}
supplement.tables.xlsx<-as_Workbook(subtable1,
                     Workbook=supplement.tables.xlsx,
                     sheet="table3")
subtable1
```
...
```{r}
supplement.tables.xlsx<-as_Workbook(subtable2,
                     Workbook=supplement.tables.xlsx,
                     sheet="table4")
subtable2
```
...
supplement.tables.xlsx<-as_Workbook(CatVar375Hux,
                     Workbook=supplement.tables.xlsx,
                     sheet="table5")
...
supplement.tables.xlsx<-as_Workbook(t6,
                     Workbook=supplement.tables.xlsx,
                     sheet="table6")
t6
```
...
supplement.tables.xlsx<-as_Workbook(t7,
                     Workbook=supplement.tables.xlsx,
                     sheet="table7")
...
supplement.tables.xlsx<-as_Workbook(t8,
                     Workbook=supplement.tables.xlsx,
                     sheet="table8")
...
We state in the text that the currently published fastqc is non-overlapping with DAIRC inclusion recommendations. Tables 9 and 10 illustrate that non-overlap.
...
```{r}
t9<-table(df$fastqcok,df$ABCD_rsfmri_QC1,dnn=c("FastQC","Tabulated")) %>% 
  as_hux() %>% 
  set_contents(1,2,"Excluded (Tabulated)") %>% 
  set_contents(1,3,"Included (Tabulated)") %>% 
  set_contents(2,1,"Excluded (FastQC)") %>% 
  set_contents(3,1,"Included (FastQC)") %>% 
  set_caption("FastQC vs Tabulated Data Inclusion/Exclusion")

supplement.tables.xlsx<-as_Workbook(t9,
                     Workbook=supplement.tables.xlsx,
                     sheet="table9")
t9
```
...
```{r}
t10<-table(df$fastqcok,df$ABCD_rsfmri_QC2,dnn=c("FastQC","DAIRC"))  %>% 
  as_hux() %>% 
  set_contents(1,2,"Excluded (DAIRC Recommended)") %>% 
  set_contents(1,3,"Included (DAIRC Recommended)") %>% 
  set_contents(2,1,"Excluded (FastQC)") %>% 
  set_contents(3,1,"Included (FastQC)") %>% 
  set_caption("FastQC vs DAIRC recommended Inclusion/Exclusion")

supplement.tables.xlsx<-as_Workbook(t10,
                     Workbook=supplement.tables.xlsx,
                     sheet="table10")
t10
```
...
t11<-performance::compare_performance(m.h2,m.h2.stripped) %>% 
  as_hux() %>% 
  set_caption("Comparison of Model Performance Indices")

supplement.tables.xlsx<-as_Workbook(t11,
                     Workbook=supplement.tables.xlsx,
                     sheet="table11")
...
See Table 12-13 for the fixed effects from the omnibus model.
...
```{r}
supplement.tables.xlsx<-as_Workbook(h2hux %>% set_caption("H2 / Omnibus model fixed effects"),
                     Workbook=supplement.tables.xlsx,
                     sheet="table12-13")
```
...
To the best of our knowledge, Gard et al. used Welch's two-sample t-tests to examine differences in propensity weight between included and excluded groups, and we will do the same here. Results from these tests are shown in table 14.
...
supplement.tables.xlsx<-as_Workbook(t14,
                     Workbook=supplement.tables.xlsx,
                     sheet="table14")
t14
```
...
Table 15 and 16 presents descriptives and a bivariate model for a SES variable inclusive of all income bins available in the ABCD dataset.
...
supplement.tables.xlsx<-as_Workbook(FullIncomeTableHux,
                     Workbook=supplement.tables.xlsx,
                     sheet="table15")
...
supplement.tables.xlsx<-as_Workbook(t16,
                     Workbook=supplement.tables.xlsx,
                     sheet="table16")
t16
```
...
Table 17 describes the many race/ethnicity endorsement variables available in ABCD, cross-referenced against the census race/ethnicity variable used in the manuscript.
...
supplement.tables.xlsx<-as_Workbook(t17,
                     Workbook=supplement.tables.xlsx,
                     sheet="table17")
...
Table 18 show descriptives of these variables by condition.
...
supplement.tables.xlsx<-as_Workbook(t18,
                     Workbook=supplement.tables.xlsx,
                     sheet="table18")
...
Finally, Tables 19-21 depict bivariate models for each race/ethnicity endorsement variable by condition.
...
supplement.tables.xlsx<-as_Workbook(remodeltable %>% set_caption("Granular Race/Ethnicity Variables: Bivariate Models"),
                     Workbook=supplement.tables.xlsx,
                     sheet="table19-21")
```
...
Here, we present the analyses originally proposed as written, excepting H2. Specifically, we present results from adjusted models without the BMI variable in the motion scrubbing conditions in Tables 22-23.
...
supplement.tables.xlsx<-as_Workbook(h1prereghux %>% set_caption("Adjusted Models Output, as Pre-registered. ***: p<.001; **: p<.01; *:p<.05"),
                     Workbook=supplement.tables.xlsx,
                     sheet="table22-23")
```
...
Supplemental tables are additionally available in an attached xlsx file.

```{r}
openxlsx::saveWorkbook(supplement.tables.xlsx,"Supplemental_Tables_editable.xlsx",overwrite=TRUE)
```
